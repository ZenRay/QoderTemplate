...
- **已存在**（GitHub Template 已克隆）→ 直接进入 Phase 3
- **不存在**（空目录）→ 执行本地模拟复制：

```bash
# 从 QoderTemplate 复制骨架（需要知道模板路径）
# 优先从以下位置自动查找：
#   ~/Documents/QoderTemplate
#   ../QoderTemplate
# 找不到则提示用户提供路径
TEMPLATE_PATH="<自动检测或用户提供>"
git archive --remote="$TEMPLATE_PATH" HEAD | tar -x -C .
# 或本地路径：
cd "$TEMPLATE_PATH" && git archive HEAD | tar -x -C <目标路径>
```
